...
createdb:
	docker exec -it postgres12alpine createdb --username=root --owner=root simple_bank

dropdb:
	docker exec -it postgres12alpine dropdb simple_bank
...
.PHONY: postgres createdb dropdb migrateup migratedown sqlc start test